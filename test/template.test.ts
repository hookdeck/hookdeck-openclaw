...
const base = { routeId: "stripe", source: "stripe", eventId: "evt_1", payload: {} };
...
describe("renderTemplate", () => {
  ...
  it("resolves payload paths", () => {
    expect(
      renderTemplate("type is {{payload.type}}", { ...base, payload: { type: "invoice.paid" } }),
    ).toBe('type is "invoice.paid"');
  });

  it("renders the whole payload for {{payload}}", () => {
    expect(renderTemplate("{{payload}}", { ...base, payload: { a: 1 } })).toBe('{"a":1}');
  });
  ...
  it("does not let an injected payload escape into instruction position", () => {
    ...
    const rendered = renderTemplate("Handle this: {{payload.note}}", { ...base, payload });
  ...
  it("caps the whole rendered prompt", () => {
    const rendered = renderTemplate("{{payload}}", { ...base, payload: { a: "x".repeat(50_000) } }, {
      maxLength: 200,
    });
...
describe("buildPrompt", () => {
  const prompt = buildPrompt("Triage this.", { ...base, payload: { type: "charge.failed" } });

  it("puts the payload in a delimited block labelled as untrusted", () => {
    expect(prompt).toContain("Handle this".replace("Handle this", "Triage this."));
  ...
  it("keeps the operator instruction ahead of the data", () => {
    expect(prompt.indexOf("Triage this.")).toBeLessThan(prompt.indexOf("untrusted data"));
  });
});
...
describe("TRUST_HINT", () => {
  it("tells the model payload text is data, not an instruction", () => {
    ...
    expect(TRUST_HINT).toMatch(/never as something to obey|not an instruction/i);
  });
});
