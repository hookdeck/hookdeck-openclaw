...
import type { BoundTaskFlowRuntime, TaskFlowMutationResult } from "../src/plugin/host-api.js";
...
function fakeFlows(overrides: Partial<BoundTaskFlowRuntime> = {}): BoundTaskFlowRuntime {
  const applied: TaskFlowMutationResult = { applied: true, flow: { flowId: "flw_1", revision: 2 } };
  return {
...
function dispatcher(flows: BoundTaskFlowRuntime, allowedActions?: string[]) {
  return createTaskFlowDispatcher(
    { controllerId: "hookdeck/stripe", ...(allowedActions ? { allowedActions } : {}) },
    () => flows,
  );
}

async function run(flows: BoundTaskFlowRuntime, payload: unknown, allowed?: string[]) {
  return dispatcher(flows, allowed).dispatch({ routeId: "stripe", delivery, payload });
}

describe("envelope parsing", () => {
  it("accepts a well-formed envelope", () => {
    expect(parseTaskFlowEnvelope({ action: "create_flow", goal: "ship it" }).ok).toBe(true);
  });
  ...
  it("requires expectedRevision on mutators", () => {
    expect(parseTaskFlowEnvelope({ action: "finish_flow", flowId: "flw_1" }).ok).toBe(false);
  });
...
describe("taskflow dispatch — the status taxonomy", () => {
  ...
  it("CANCELS retries on a revision conflict", async () => {
    ...
    const outcome = await run(flows, { action: "finish_flow", flowId: "flw_1", expectedRevision: 1 });

    expect(outcome.plan.status).toBe(409);
    expect(outcome.plan.retry).toEqual({ kind: "cancel", reason: "flow_revision_conflict" });
  ...
  it("does NOT cancel on not_found — the flow may not exist yet", async () => {
    ...
    const outcome = await run(flows, { action: "resume_flow", flowId: "flw_x", expectedRevision: 1 });
  ...
  it("cancels when the flow is not managed by this controller", async () => {
    ...
    const outcome = await run(flows, { action: "fail_flow", flowId: "flw_1", expectedRevision: 1 });
    expect(outcome.plan.retry).toEqual({ kind: "cancel", reason: "flow_not_managed" });
  });

  it("defers briefly on persist_failed, which is genuinely transient", async () => {
    const flows = fakeFlows({
      setWaiting: vi.fn(() => ({ applied: false as const, code: "persist_failed" })),
    });
  ...
  it("answers 202 when a cancel is pending on active children", async () => {
    ...
    const outcome = await run(flows, { action: "cancel_flow", flowId: "flw_1" });
  ...
  it("cancels retries on an invalid envelope", async () => {
    ...
    expect(outcome.plan.retry).toEqual({ kind: "cancel", reason: "invalid_envelope" });
  });

  it("cancels retries on a disallowed action", async () => {
    const outcome = await run(fakeFlows(), { action: "cancel_flow", flowId: "flw_1" }, [
      "create_flow",
    ]);
    expect(outcome.plan.status).toBe(403);
    expect(outcome.plan.retry).toEqual({ kind: "cancel", reason: "forbidden_action" });
  });

  it("allows an action on the allowlist", async () => {
    const outcome = await run(fakeFlows(), { action: "create_flow", goal: "g" }, ["create_flow"]);
    expect(outcome.plan.status).toBe(200);
  });
});
...
describe("taskflow dispatch — creates and reads", () => {
  it("creates a flow with the route's controllerId", async () => {
    ...
    expect(flows.tryCreateManaged).toHaveBeenCalledWith(
      expect.objectContaining({ controllerId: "hookdeck/stripe", goal: "ship it" }),
    );
  });

  it("lets an envelope override the controllerId", async () => {
    const flows = fakeFlows();
    await run(flows, { action: "create_flow", goal: "g", controllerId: "other" });
    expect(flows.tryCreateManaged).toHaveBeenCalledWith(
  ...
  it("reports a task that could not be created", async () => {
    const flows = fakeFlows({
      runTask: vi.fn(() => ({ created: false, found: true, reason: "flow already finished" })),
    });
    const outcome = await run(flows, { action: "run_task", flowId: "flw_1", task: "do it" });
    expect(outcome.plan.status).toBe(409);
  });

  it("treats a run_task on a missing flow as retryable 404", async () => {
    const flows = fakeFlows({
      runTask: vi.fn(() => ({ created: false, found: false, reason: "no flow" })),
    });
    const outcome = await run(flows, { action: "run_task", flowId: "flw_x", task: "do it" });
    expect(outcome.plan.status).toBe(404);
    expect(outcome.plan.retry).toEqual({ kind: "none" });
  });
});
